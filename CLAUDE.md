...
Context and conventions for Claude Code when working on the VAY Sports Fest integration. Read this and the referenced skill before writing or modifying any code in this repo.
...
When Claude Code creates new files, they go in the language-appropriate tier — Python under `middleware/`, PHP under `plugins/`. Do not create a `src/` at the repo root; that's not the convention here.
