...
| Team-code-filled visual schedule | July 2026 | Issue #214 added `import-match-schedule-overrides` for visual BB/MVB/WVB team-code rows that specify both where a slot is and which teams occupy it. | Team-code-filled visual schedule with audit before overwrite |
| Approved preliminary game publication | July 2026 | Issue #217 added `import-approved-games` to normalize the four approved preliminary-game workbooks into stable `game_key` records and publish-ready JSON for WordPress score entry. | Approved XLSX workbooks plus guarded `publish-schedule` |
...
| WordPress score-entry preliminary games | `approved_schedule_games.json` generated by `import-approved-games` | Stable `game_key` records from approved preliminary-game workbooks; published through generated `approved_schedule_input.json` / `approved_schedule_output.json`. |
...
### Approved preliminary game import for WordPress
...
- `COED SOCCER SCHEDULE.xlsx` for Soccer G1-G6 exact games while preserving
  G7-G12 as unresolved placeholders;
- `Schedule_Roster - Table Tennis (PingPong) 2026.xlsx` for Table Tennis
  preliminary games and category identity.

The command writes an audit on dry run and, on clean execute, writes:

- `approved_schedule_games.json`
- `approved_schedule_input.json`
- `approved_schedule_output.json`

The approved input/output pair is intentionally separate from the ordinary
solver files. It contains the exact games that should be available for
WordPress score entry and lets `publish-schedule` remain the only code path
...
Execution is blocked when an exact game cannot resolve to the current
resources, when game keys or resource slots duplicate, or when the known Table
Tennis U35 `SBC` roster/schedule discrepancy is present without an explicit
`--waive-table-tennis-discrepancy` operator decision.
